...
const infos = [
  ...
  { icon: Clock, label: 'Horaires', value: '10h00 (enfants) · 18h30 (adultes) → Minuit' },
  { icon: Euro, label: 'Tarif', value: 'Entrée gratuite — restauration payante' },
  { icon: Accessibility, label: 'Accessibilité', value: 'Site entièrement accessible PMR' },
  { icon: Car, label: 'Accès', value: 'Parking de la Hotoie (gratuit le soir) · Bus lignes 3, 7, 15' },
]

export default function InfosPratiques() {
  return (
    <section id="infos" className="py-24 px-6 md:px-12 lg:px-16" style={{ background: '#000' }}>
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <FadeIn fromScroll delay={200} duration={700}>
            <div className="flex flex-col gap-4" id="inscriptions">
              {infos.map((info, i) => {
                const Icon = info.icon
                return (
                  <div
                    key={i}
                    className="liquid-glass border border-white/10 rounded-xl px-5 py-4 flex items-start gap-4 hover:border-white/25 transition-all duration-300"
                  >
                    <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5 border border-white/10">
